...
pos_all=pos(in,:);r_pos_all=pos_all;
% plot(domain);plot(pos_all(:,1),pos_all(:,2),'.b');axis equal;axis off;
% visual_poly(point);axis equal;axis off;
sarea=polyarea(point(:,1),point(:,2));
%
% addpath('C:\Users\m-q-x\OneDrive\Code\rand_comp\2d');
dr=0.25;   eta=0.0; n1=8;  n2=12;  ext=1.25;
rot=-1;distance=0.1;
rgrade=[2.5 1.5 0.75 0.5];frac=[65 25 10];fraction=50;
% fraction=30;
Polygon=cell(1000,1);
particlenum=0;
Tsum=0;
...
% patch('faces',tria(tnum==1,1:3),'vertices',vert,'facecolor',color(1,:),   'edgecolor',color(2,:)) ;
% hold on; axis image off;
% patch('faces',tria(tnum==2,1:3),'vertices',vert,  'facecolor',color(3,:), 'edgecolor',color(4,:)) ;
% patch('faces',edge(:,1:2),'vertices',node,   'facecolor','w',  'edgecolor',color(5,:),  'linewidth',1.5) ;
% % title(['MESH-OPT.: KIND=DELFRONT, |TRIA|=',   num2str(size(tria,1))]) ;
% title(['FEM MESH WITH ',   num2str(size(tria,1)), ' ELEMENTS']) ;
patch('faces',tria(tnum==1,1:3),'vertices',vert,'facecolor',[1.,1.,1.],   'edgecolor',[.2,.2,.2]) ;
hold on; axis image off;
patch('faces',tria(tnum==2,1:3),'vertices',vert,  'facecolor',[.9,.9,.9], 'edgecolor',[.2,.2,.2]) ;
patch('faces',edge(:,1:2),'vertices',node,   'facecolor','w',  'edgecolor',[.1,.1,.1],  'linewidth',1.5) ;
% % title(['MESH-OPT.: KIND=DELFRONT, |TRIA|=',   num2str(size(tria,1))]) ;
% title(['FEM MESH WITH ',   num2str(size(tria,1)), ' ELEMENTS']) ;
...
disp(['The safety factor is ',num2str(fos_down)]);
