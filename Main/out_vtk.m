function out_vtk(file,vert,tria,tnum,disps,stress,strain)
%Out information to vtk
g_coord=vert';
g_num=[tria(:,1),tria(:,3),tria(:,2)]';
fid=fopen(file,'wt');%'myslope.vtk'
...
% fprintf(fid,'VECTORS pstress float\n');
% for i=1:nn
%     fprintf(fid,'%f %f %f\n',nstress(1,i),nstress(2,i),nstress(3,i));
% end
% fprintf(fid,'VECTORS pstrain float\n');
% for i=1:nn
